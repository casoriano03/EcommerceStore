...
onMounted(async()=>{
  ...
  if (tokenPresent) {
    ...
    userId.value = parseInt(decodedPayload.nameid);
    ...
    try {
      const cartItems = await axios.get('https://localhost:7023/api/Cart/GetCartItemByUserId', {
      params:{id:userId.value}
      });
      globalState.carts=cartItems.data;
      globalState.carts.forEach(item => {
      globalState.itemCount += item.quantity;
      globalState.cartTotal += item.quantity*item.product.price
    });
    } catch (error) {
      console.log(error)
    }
  ...
  <div class="container-fluid navContainer">
    <header class="d-flex flex-wrap justify-content-center py-3 border-bottom">
      ...
      <ul class="nav nav-pills me-3">
        ...
        <li class="nav-item fs-5 pt-1">${{ globalState.cartTotal.toFixed(2) }}</li>
        <li class="nav-item"><RouterLink to="/cart" class="nav-link"><div class="border border-dark ps-2 pe-2 fs-6 mt-0 text-dark bg-light">{{ globalState.itemCount }}</div></RouterLink></li>
        <li class="nav-item">
          <RouterLink to="/login" class="nav-link dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false"><img :src="isUserLoggedIn?loggedUserIcon:defaultUserIcon" alt="user icon" height="20"></RouterLink>
          <ul class="dropdown-menu text-center">
            <li><a v-if="!isLoggedIn" class="dropdown-item"><RouterLink to="/login">Login</RouterLink></a></li>
            <li><a v-if="!isLoggedIn" class="dropdown-item"><RouterLink to="/register">Register</RouterLink></a></li>
            <li><a v-if="isLoggedIn" class="dropdown-item"><RouterLink to="/profile">Profile</RouterLink></a></li>
            <li><a v-if="isAdmin" class="dropdown-item"><RouterLink to="/admin"> Admin</RouterLink></a></li>
